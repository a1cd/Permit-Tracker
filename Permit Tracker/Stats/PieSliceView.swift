	...
	var body: some View {
		GeometryReader { geometry in
			ZStack {
				...
				Image(systemName: pieSliceData.text)
...
struct PieSlice_Previews: PreviewProvider {
	static var previews: some View {
		PieSlice(pieSliceData: PieSliceData(startAngle: Angle(degrees: 0.0), endAngle: Angle(degrees: 120.0), text: "30%", icon: "sunset.fill", color: Color.black))
	}
}
